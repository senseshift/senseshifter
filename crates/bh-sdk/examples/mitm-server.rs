...
impl MitmServer {
    ...
    async fn connect_to_server(&self, conn_id: &str, query_params: &str) -> Result<(tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<TcpStream>>, [u8; 32])> {
        ...
        info!("[{}] Connecting to target server: {}", conn_id, server_url_with_params);
                ...
                Message::Text(text) => {
                    if let Ok(sdk_msg) = serde_json::from_str::<SdkEncryptedMessage>(&text) {
                        match sdk_msg.r#type() {
                            SdkEncryptedMessageType::ServerKey => {
                                if let Some(key_b64) = sdk_msg.key() {
                                    ...
                                    let ws_stream = write.reunite(read).map_err(|e| anyhow!("Failed to reunite streams: {}", e))?;
    ...
    async fn handle_client(&self, stream: TcpStream, addr: SocketAddr) -> Result<()> {
        ...
        let callback = move |req: &Request, response: Response| {
            if let Some(query_string) = req.uri().query() {
                if let Ok(mut params) = query_params_clone.try_lock() {
                    *params = query_string.to_string();
                    info!("[{}] Extracted query params: {}", conn_id_for_callback, query_string);
                }
            }
            Ok(response)
        };
        ...
        client_write.send(Message::Text(server_key_json.into())).await?;
                            ...
                            SdkEncryptedMessageType::SdkClientKey => {
                                if let Some(encrypted_key_b64) = sdk_msg.key() {
                                    ...
                                    match self.decrypt_client_key_pkcs1v15(encrypted_key_b64) {
                                        Ok(client_aes_key) => {
                                            info!("[{}] Client AES key: {}", conn_id, hex::encode(client_aes_key));
                                            ...
                                            match self.connect_to_server(&conn_id, &query_params).await {
                                                Ok((server_ws, server_aes_key)) => {
                                                    info!("[{}] Server AES key: {}", conn_id, hex::encode(server_aes_key));
                                                    ...
                                                    info!("[{}] MITM setup complete! Ready to intercept messages", conn_id);
                                                    
                                                    // Start message interception loop
                                                    return self.start_message_interception(conn_id.clone(), client_write, client_read, server_ws).await;
                                                }
                                                Err(e) => {
    ...
    async fn start_message_interception(
        &self,
        conn_id: String,
        mut client_write: futures_util::stream::SplitSink<tokio_tungstenite::WebSocketStream<TcpStream>, Message>,
        mut client_read: futures_util::stream::SplitStream<tokio_tungstenite::WebSocketStream<TcpStream>>,
                                                ...
                                                match conn.client_crypto.decrypt_aes_gcm(encrypted_data) {
                                                    Ok(plaintext) => {
                                                        ...
                                                        match conn.server_crypto.encrypt_aes_gcm(&plaintext) {
                                                            Ok(server_encrypted) => {
                                                                let server_msg = SdkEncryptedMessage::new(
                                                                    SdkEncryptedMessageType::SdkData,
                                                                    None,
                                                                    Some(server_encrypted),
                                                                );
                                                ...
                                                match conn.server_crypto.decrypt_aes_gcm(encrypted_data) {
                                                    Ok(plaintext) => {
                                                        ...
                                                        match conn.client_crypto.encrypt_aes_gcm(&plaintext) {
                                                            Ok(client_encrypted) => {
                                                                let client_msg = SdkEncryptedMessage::new(
                                                                    SdkEncryptedMessageType::SdkData,
                                                                    None,
                                                                    Some(client_encrypted),
                                                                );
...
async fn main() -> Result<()> {
    ...
    if args.len() != 3 {
        ...
        eprintln!("Example: {} 8080 \"ws://127.0.0.1:15881/v4/feedback\"", args[0]);
        return Ok(());
    }
