...
pub trait Mapper {
  fn map(&self, x: f32, y: f32, max_points: usize) -> HashMap<usize, f64>;
}
...
/// ```
pub struct InterpolatingMapperEvenGrid {
  x_points: usize,
  y_points: usize,
}

impl InterpolatingMapperEvenGrid {
  pub fn new(x_points: usize, y_points: usize) -> Self {
    Self { x_points, y_points }
  }
}

impl Mapper for InterpolatingMapperEvenGrid {
  ...
  fn map(&self, x: f32, y: f32, max_points: usize) -> HashMap<usize, f64> {
    ...
    let closest = &distances[..max_points.min(distances.len())];
...
pub struct InterpolatingMapperWithCoordinates {
  ...
  grid_points: Vec<(f32, f32)>,
}

impl InterpolatingMapperWithCoordinates {
  pub fn new(grid_points: Vec<(f32, f32)>) -> Self {
    Self { grid_points }
  }
}
